# -*- coding: UTF-8 -*-
"""
listed_company_executive_crawler.middlewares
============================
自定义中间件

功能特性:
- IP代理中间件：支持静态代理列表和动态代理API
- 时间窗口失效检测：5分钟内失败3次标记为坏代理
- 自动从失效列表中恢复过期的代理
- 智能重试：前2次使用代理，第3次使用本机IP
"""
...
class ProxyMiddleware:
    ...
    def __init__(self, settings):
        ...
        if self.proxies:
            self.mode = "static"
            self.enabled = True
            if not ProxyMiddleware._initialized:
                self.logger.info(f"代理中间件已启用 (静态模式) | 代理数量: {len(self.proxies)}")
                ProxyMiddleware._initialized = True
        elif self.api_url:
            self.mode = "dynamic"
            self.enabled = True
            if not ProxyMiddleware._initialized:
        ...
        if isinstance(data, dict):
            ...
            for field in ['http', 'https', 'url', 'address']:
                if field in data:
                    ...
                    if isinstance(proxy_value, str) and (
                            proxy_value.startswith('http://') or proxy_value.startswith('https://')):
                        return proxy_value
    ...
    async def process_request(self, request: Request, spider) -> Optional[Request]:
        ...
        if request.proxy is not None:
            # 请求已指定代理（包括 None 表示直连），不覆盖
            return None

        # 检查重试次数，与 RetryMiddleware 的代理切换逻辑保持一致
        # proxy_switch_threshold = max(1, (max_retry_times + 1) // 2)
        # 默认 max_retry_times=3，则 threshold=2
        retry_times = request.meta.get('retry_times', 0)
        proxy_switch_threshold = 2  # 与框架默认配置保持一致
        
        # 超过阈值时，不分配代理，使用本机IP直连
        if retry_times > proxy_switch_threshold:
            self.logger.info(
                f"重试次数({retry_times})超过代理切换阈值({proxy_switch_threshold})，"
                f"使用本机IP直连: {request.url}"
            )
            return None
        ...
        if proxy:
            request.proxy = proxy
            self.logger.debug(f"为 {request.url} 分配代理 {proxy} (重试次数: {retry_times})")
        else:
