...
class TelecomDeviceSpider(Spider):
    ...
    def parse(self, response):
        ...
        logger.info(f"正在解析第 {page} 页，状态码: {response.status_code}")
        
        try:
            json_data = response.json()
            
            if not json_data.get('success'):
                logger.error(f"第 {page} 页请求失败: {json_data.get('msg', 'Unknown error')}")
                return
            
            # 提取总页数和总记录数（可选，用于验证）
            total_records = json_data.get("params", {}).get("tbAppArticle", {}).get("total", 0)
            logger.info(f"第 {page} 页，总记录数: {total_records}")
            ...
            logger.info(f"第 {page} 页成功解析到 {len(article_list)} 条记录")
            ...
            if page < self.end_page:
                ...
                logger.debug(f"准备爬取下一页: {next_page}")
                yield Request(
                    url=self.base_api_url,
                    method='POST',
                    headers=HEADERS,
