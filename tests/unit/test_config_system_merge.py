...
class TestImportCorrectness(unittest.TestCase):
    ...
    def test_config_manager_module_removed_in_v2(self):
        """v2.0: crawlo.utils.config_manager 已物理删除，import 抛 ModuleNotFoundError"""
        with self.assertRaises(ModuleNotFoundError):
            import crawlo.utils.config_manager  # noqa: F401
...
class TestBackwardCompatibility(unittest.TestCase):
    """v2.0: 验证旧路径 config_manager 已物理删除"""

    def test_configutils_identity(self):
        """v2.0: ConfigUtils 从新路径 crawlo.utils.misc 导入正常"""
        from crawlo.utils.misc import ConfigUtils
        self.assertTrue(isinstance(ConfigUtils, type))

    def test_envconfigmanager_identity(self):
        """v2.0: EnvConfigManager 从新路径 crawlo.settings.setting_manager 导入正常"""
        from crawlo.settings.setting_manager import EnvConfigManager
        self.assertTrue(isinstance(EnvConfigManager, type))

    def test_configutils_methods_accessible_from_new_path(self):
        """新路径的 ConfigUtils 方法可调用"""
        from crawlo.utils.misc import ConfigUtils
        result = ConfigUtils.get_config_value([{'A': 'hello'}], 'A')
        self.assertEqual(result, 'hello')

    def test_envconfigmanager_methods_accessible_from_new_path(self):
        """新路径的 EnvConfigManager 方法可调用"""
        from crawlo.settings.setting_manager import EnvConfigManager
