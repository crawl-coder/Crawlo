...
# 确保项目根目录在 path 中
# 文件位于 tests/integration/ 下；dirname 两次 => tests/，再一次 => 项目根（setup.cfg 所在）
...
async def test_ack_message_logs_on_failure():
    ...
    await _ack_message(FakeRequest(), engine, success=True)
...
@pytest.mark.asyncio
async def test_semaphore_released_on_invalid_dequeue():
