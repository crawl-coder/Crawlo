...
class TestOptimization_5_1_CloseSpiderIdempotency:
    ...
    async def test_close_spider_resets_flag_on_exception(self):
        ...
        # Phase 3：检查点三方法已迁入 CheckpointCoordinator（self._checkpoint）
        # _init_configs 已初始化 _checkpoint，此处覆盖一个能穿透 close_spider try 块的 mock
        # clear_checkpoint 内部有 try/except 不会穿透，故 mock save_checkpoint 抛异常
        engine._checkpoint = Mock()
        engine._checkpoint.clear_checkpoint = AsyncMock(side_effect=RuntimeError("cleanup failed"))
    ...
    async def test_close_spider_idempotent_on_success(self):
        ...
        engine._init_configs()
        engine.task_manager = None
        # Phase 3：_shutdown_cluster 依赖 _cluster_state.worker_id（ClusterState 默认 None）
...
class TestOptimization_3_2_SchedulerConditionWait:
    """验证 Condition 替代 sleep 轮询（Phase 2：已下沉到 QueueManager）"""

    def test_has_queue_not_full_condition(self):
        """QueueManager 应有 _queue_not_full = asyncio.Condition()（Phase 2 下沉）"""
        from crawlo.queue.queue_manager import QueueManager
        # 检查 __init__ 中是否有 _queue_not_full
        import inspect
        source = inspect.getsource(QueueManager.__init__)
        assert '_queue_not_full' in source, "QueueManager.__init__ 应包含 _queue_not_full"
        assert 'Condition' in source, "QueueManager 应使用 asyncio.Condition"

    def test_put_uses_condition_wait(self):
        """QueueManager.put 应使用 Condition 等待而非 sleep"""
        import inspect
        from crawlo.queue.queue_manager import QueueManager
    ...
    def test_get_notifies(self):
        """QueueManager.get 应在出队后 notify_all（Phase 2 下沉）"""
        import inspect
        from crawlo.queue.queue_manager import QueueManager
        source = inspect.getsource(QueueManager.get)
        assert 'notify_all' in source or '_notify_space_available' in source, \
            "QueueManager.get 应调用 _notify_space_available 通知等待的入队协程"
