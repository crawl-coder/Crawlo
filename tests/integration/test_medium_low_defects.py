...
- MEDIUM-5: QueueManager.async_empty() 队列空检查
...
class TestMedium5RedisEmpty:
    """测试 MEDIUM-5: QueueManager.async_empty() 队列空检查"""

    @pytest.mark.asyncio
    async def test_async_empty_returns_false_for_nonempty_redis_queue(self):
        """Redis 队列非空时 async_empty() 返回 False"""
        ...
        with patch.object(QueueManager, '__init__', lambda self, **kwargs: None):
            ...
            result = await qm.async_empty()
            assert result is False, (
                "Redis 队列非空时 async_empty() 应返回 False，避免过早退出"
            )

    @pytest.mark.asyncio
    async def test_async_empty_returns_true_for_empty_memory_queue(self):
        """内存队列空时 async_empty() 返回 True"""
        from crawlo.queue.queue_manager import QueueManager, QueueType
        ...
        with patch.object(QueueManager, '__init__', lambda self, **kwargs: None):
            ...
            qm._queue.size = AsyncMock(return_value=0)
