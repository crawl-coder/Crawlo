...
class TestPhase0_Infrastructure:
    ...
    def test_context_fields_complete(self):
        ...
        # Phase 1: 核心注册表（RegistryContext）
        assert hasattr(ctx.registries, 'component_registry')
        assert hasattr(ctx.registries, 'initializer_registry')
        assert hasattr(ctx.registries, 'spider_registry')
        assert hasattr(ctx.registries, 'job_registry')
        assert hasattr(ctx.registries, 'framework')

        # Phase 2: 框架管理器（RuntimeContext）
        # 注：LogManager 按 Phase 4 计划保持 SingletonMeta 独立（进程级资源），
        # 不迁入 ctx，故不在此断言。
        assert hasattr(ctx.runtime, 'error_handler_instance')
        assert hasattr(ctx.runtime, 'performance_monitor')
        assert hasattr(ctx.runtime, 'resource_managers')
        assert hasattr(ctx.runtime, '_monitor_manager')

        # Phase 3: Bot 通知（NotificationContext）
        assert hasattr(ctx.notifications, 'notifier')
        assert hasattr(ctx.notifications, 'notifier_lock')
        assert hasattr(ctx.notifications, 'notification_handler')
        assert hasattr(ctx.notifications, 'template_manager')
        assert hasattr(ctx.notifications, 'deduplicator')
        assert hasattr(ctx.notifications, 'dingtalk_channel')
        assert hasattr(ctx.notifications, 'feishu_channel')

        # Phase 4: MCP/工具（RuntimeContext）
        assert hasattr(ctx.runtime, 'quick_fetcher')
        assert hasattr(ctx.runtime, 'mcp_fetcher')
        assert hasattr(ctx.runtime, 'redis_manager')
        assert hasattr(ctx.runtime, 'connection_pools')
...
class TestPhase2_FrameworkManagers:
    ...
    def test_error_handler(self):
        """测试 ErrorHandler 迁移"""
        from crawlo.utils.errors import _get_global_error_handler
...
class TestIntegration:
    ...
    def test_context_cleanup(self):
        ...
        assert "test_resource" in ctx.runtime.resources

        ctx.remove_resource("test_resource")
        assert "test_resource" not in ctx.runtime.resources
