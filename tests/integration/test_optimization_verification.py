...
class TestSchedulerConditionVariable:
    """测试 Condition 变量机制（Phase 2：已下沉到 QueueManager）"""
    ...
    def scheduler(self):
        ...
        sched.queue_manager.config = Mock(
            enqueue_full_policy='drop_with_counter',
            enqueue_block_timeout=None,
            enqueue_drop_timeout=50.0,
            queue_name='test',
            max_queue_size=10,
        )
        sched.crawler = Mock()
    ...
    def test_has_queue_not_full_condition(self, scheduler):
        """QueueManager 应有 _queue_not_full Condition 变量（Phase 2 下沉）"""
        ...
        assert hasattr(qm, '_queue_not_full'), \
            "QueueManager 应有 _queue_not_full Condition 变量"
        assert isinstance(qm._queue_not_full, asyncio.Condition), \
    ...
    @pytest.mark.asyncio
    async def test_put_uses_condition(self):
        """QueueManager.put 应使用 Condition 等待（Phase 2 下沉）"""
        from crawlo.queue.queue_manager import QueueManager
        ...
        assert "_wait_for_space" in source, \
            "QueueManager.put 应调用 _wait_for_space 进行阻塞等待"
