...
class MySQLHelper:
    ...
    async def insert_many(
        ...
        for i in range(0, len(datas), batch_size):
            ...
            # 构建批量插入 SQL
            sql, params = self._build_batch_insert(
                table_name.strip("`"),
                batch,
                auto_update=auto_update,
                update_columns=update_columns,
    ...
    def _build_batch_insert(
        ...
        keys_str = ", ".join(f"`{k}`" for k in columns)
        ...
        for data in datas:
            ...
            params.extend([data.get(k) for k in columns])
        
        values_str = ", ".join(placeholders_list)
        table_fmt = f"`{table}`"
        
        # 构建完整 SQL
        if auto_update:
            sql = f"REPLACE INTO {table_fmt} ({keys_str}) VALUES {values_str}"
        elif insert_ignore:
            sql = f"INSERT IGNORE INTO {table_fmt} ({keys_str}) VALUES {values_str}"
        elif update_columns:
            update_clause = ", ".join(f"`{col}`=VALUES(`{col}`)" for col in update_columns)
            sql = f"INSERT INTO {table_fmt} ({keys_str}) VALUES {values_str} ON DUPLICATE KEY UPDATE {update_clause}"
        else:
            sql = f"INSERT INTO {table_fmt} ({keys_str}) VALUES {values_str}"
...
async def get_mysql_helper(settings=None, spider=None) -> MySQLHelper:
    """
    获取 MySQLHelper 实例
    ...
    Example:
        # 方式1: 使用 settings（原有方式）
        helper = await get_mysql_helper(settings)
        
        # 方式2: 直接传入 spider（推荐）
        # spider 需要有 custom_settings = {'MYSQL_TABLE': 'ofweek_news'}
        helper = await get_mysql_helper(spider=self)
    """
...
async def check_exists(
    ...
    """
    快速检查数据是否存在（单例模式，适合批量场景）
    
    Example:
        # 批量场景中复用同一实例
        helper = await get_mysql_helper()
        
        for item in items:
            if await helper.exists("articles", {"url": item["url"]}):
                continue
            # 处理数据...
    """
