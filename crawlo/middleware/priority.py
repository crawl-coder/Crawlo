...
优先级规则（实际执行序，由 tests/arch/test_middleware_order_contract.py 锁定）：
- 构建顺序：按优先级数值降序排列
- 请求阶段（process_request）：数值越大越先执行
- 响应阶段（process_response）：数值越小越先执行
- 异常阶段（process_exception）：与请求阶段同序，首个返回 Request/Response 者接管
