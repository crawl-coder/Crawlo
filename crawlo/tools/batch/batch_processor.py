...
class BatchProcessor:
    ...
    async def process_batch(self, items: List[Any], processor_func: Callable, 
        ...
        """
        # 为了向后兼容，仍然提供实现
        from crawlo.tools.batch.batch_manager import BatchProcessor as NewBatchProcessor
...
class RedisBatchProcessor:
    ...
    async def batch_set(self, items: List[Dict[str, Any]]) -> int:
        ...
        """
        # 为了向后兼容，仍然提供实现
        from crawlo.tools.batch.batch_manager import RedisBatchProcessor as NewRedisBatchProcessor
...
async def process_in_batches(items: List[Any], processor_func: Callable,
    ...
    便利函数：分批处理大量数据项（已弃用）
    """
    from crawlo.tools.batch.batch_manager import process_in_batches as new_process_in_batches
...
def batch_process(batch_size: int = 100, max_concurrent_batches: int = 5):
    ...
    装饰器：将函数转换为批处理函数（已弃用）
    """
    from crawlo.tools.batch.batch_manager import batch_process as new_batch_process
