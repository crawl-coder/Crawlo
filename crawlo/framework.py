...
v2.0 前发出 DeprecationWarning，并通过 sys.modules 导出 CrawloFramework + 便捷函数。
"""
import sys
import warnings
import importlib
...
if __name__ != _NEW_MODULE:
    ...
    # 重新注册本模块 → 指向新模块（使 from crawlo.framework import xxx 等价）
    sys.modules[__name__] = _new
    warnings.warn(
        f"{__name__} is deprecated, use {_NEW_MODULE} instead (CrawloFramework / run_spider now live in crawlo.crawler)",
        DeprecationWarning,
        stacklevel=2,
    )
