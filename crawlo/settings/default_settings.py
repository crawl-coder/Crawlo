...
# 内存队列配置（适用于单机小规模）
MEMORY_SCHEDULER_MAX_QUEUE_SIZE = 5000  # 内存队列最大大小

# Redis队列配置（适用于分布式大规模）
REDIS_SCHEDULER_MAX_QUEUE_SIZE = 50000  # Redis队列最大大小（更大容量）
...
# 内存队列背压配置（适用于单机小规模）
# 特点：访问速度快（纳秒级），队列容量小，可以容忍更高的使用率
MEMORY_BACKPRESSURE_RATIO = 0.8  # 内存队列背压触发阈值（80%）
MEMORY_BACKPRESSURE_DELAY_BASE = 0.2  # 内存队列基础延迟（秒）
MEMORY_BACKPRESSURE_DELAY_MAX = 2.0  # 内存队列最大延迟（秒）
MEMORY_BACKPRESSURE_WARNING_THRESHOLD = 0.7  # 内存队列警告阈值（70%）
MEMORY_BACKPRESSURE_CRITICAL_THRESHOLD = 0.9  # 内存队列危险阈值（90%）
