...
# ----- 2.1 队列大小配置 -----
# 通用配置（向后兼容）
SCHEDULER_MAX_QUEUE_SIZE = 10000  # 调度器队列最大大小（向后兼容，将被队列类型特定配置覆盖）

# 内存队列配置（适用于单机小规模）
# 优化：从 5000 降到 3000，配合 60% 背压阈值，在 1800 pending 时触发背压
# 防止队列积压过快增长导致内存溢出，同时保持足够的处理能力
MEMORY_SCHEDULER_MAX_QUEUE_SIZE = 3000  # 内存队列最大大小（优化：更早触发保护）

# Redis队列配置（适用于分布式大规模）
# 优化：从 50000 降到 10000，配合 60% 背压阈值，在 6000 pending 时触发背压
# Redis 虽然容量大，但过大的队列会导致：
# 1. 背压触发太晚，失去保护意义
# 2. Redis 内存占用过高（50-250MB）
# 3. 网络延迟累积，影响性能
REDIS_SCHEDULER_MAX_QUEUE_SIZE = 10000  # Redis队列最大大小（优化：更早触发保护）
