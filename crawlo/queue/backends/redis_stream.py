...
class RedisStreamQueue:
    ...
    async def size(self) -> int:
        """获取队列历史消息总数（用于监控，不用于 idle 判断）
        
        注意：Stream 消费后消息不会自动删除，仅 maxlen 裁剪。
        因此 size() 返回的是历史消息数，不是"待消费数"。
        idle/empty 判断请使用 empty()。
        """
        self._ensure_connected()
        total = 0
        try:
    ...
    async def empty(self) -> bool:
        """检查队列是否真正空闲（无可消费消息 + 无 Pending 未 ACK）

        Stream 语义下，"空" 必须同时满足：
          1. Consumer Group 的 pending == 0（没有拿到但未 ACK 的消息）
          2. last-delivered-id >= last-generated-id（消费进度已经追上最新）
             即 last-delivered-id 之后没有新消息
        """
        self._ensure_connected()
        try:
            streams = {self._stream, self._high_stream}
            for stream in streams:
                try:
                    ...
                    last_gen = stream_info.get('last-generated-id')
                    ...
                    for g in groups:
                        ...
                        if last_gen and last_delivered:
