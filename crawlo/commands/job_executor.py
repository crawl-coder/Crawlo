...
调用 L2 的 CrawlerProcess 启动单次爬虫执行，并处理日志重配、并发控制、超时、重试。

旧位置 ``crawlo.scheduling.daemon.executor`` 已改为 re-export facade + DeprecationWarning；
新代码请直接 ``from crawlo.commands.job_executor import JobExecutor``。
"""
...
class JobExecutor:
    """定时任务执行器（L1 commands 层，替代 crawlo.scheduling.daemon.executor.JobExecutor）。

    与旧类 API 完全兼容：``__init__(settings, stats, logger)`` + ``init_concurrency``
    + ``execute_with_semaphore(job)`` + ``execute_job(job)`` + ``add_task(task)``
    + ``running_tasks`` 属性。
    """
    ...
    async def execute_job(self, job: ScheduledJob):
        ...
        try:
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(job_task, timeout=timeout)
