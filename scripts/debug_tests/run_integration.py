...
async def main():
    ...
    try:
        ...
        if engine:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Cluster worker_id: {engine._cluster_state.worker_id}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster registry: {engine._cluster_state.registry is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster heartbeat: {engine._cluster_state.heartbeat is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster failover: {engine._cluster_state.failover is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster progress: {engine._cluster_state.progress is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster rate_limiter: {engine._cluster_state.rate_limiter is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster monitor: {engine._cluster_state.monitor is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster messenger: {engine._cluster_state.messenger is not None}")
            print(f"[{time.strftime('%H:%M:%S')}] Cluster dynamic_config: {engine._cluster_state.dynamic_config is not None}")
        else:
