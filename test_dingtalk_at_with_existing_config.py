...
"""
钉钉通知 @ 功能测试脚本（使用现有配置）
使用 settings.py 中的钉钉机器人配置进行 @ 功能测试
"""
...
def test_dingtalk_at_with_existing_config():
    """使用现有配置测试钉钉 @ 功能"""
    print("🔧 配置钉钉通知渠道（使用现有配置）...")
    ...
    phone_number = "15361276730"  # settings.py 中的电话号码
    
    # 获取钉钉渠道实例并配置，使用关键词 "爬虫" 并设置@功能
    dingtalk_channel = get_dingtalk_channel()
    
    print("\n--- 测试1: @特定手机号（来自settings.py）---")
    # 配置@特定手机号
    dingtalk_channel.set_config(
        webhook_url=webhook_url, 
        secret=secret, 
        keywords=["爬虫"],
        at_mobiles=[phone_number]  # 使用 settings.py 中的手机号
    )
    ...
    print("\n" + "="*70)
    print("🎉 钉钉@功能测试总结:")
    print(f"✅ 使用现有配置: Webhook URL 和 Secret 来自 settings.py")
    print(f"✅ @手机号测试: 使用 settings.py 中的手机号 {phone_number}")
...
if __name__ == "__main__":
    test_dingtalk_at_with_existing_config()