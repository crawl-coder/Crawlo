...
# 检查点存储目录（默认 ~/.crawlo/checkpoints/{project}/{spider}）
CHECKPOINT_DIR = None
...
```bash
# 通过配置项禁用
crawlo run myspider -s CHECKPOINT_ENABLED=False
```
...
1. 检查配置：`CHECKPOINT_ENABLED = True`
2. 检查关闭原因：必须是 `reason='shutdown'`（Ctrl+C）
3. 查看日志：搜索 `Checkpoint saved` 关键字

### 检查点未恢复

1. 确认检查点文件存在：`ls ~/.crawlo/checkpoints/{project}/{spider}.json`
