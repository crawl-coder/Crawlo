...
- **内存模式**：单机运行（内存队列），`RUN_MODE='standalone'` `QUEUE_TYPE='memory'`
- **多节点协作**：多机并发（Redis ZSET），`RUN_MODE='auto'` `QUEUE_TYPE='redis'`
- **分布式系统**：生产级（Redis Stream + ACK + 故障转移），`RUN_MODE='distributed'` `QUEUE_TYPE='redis_stream'`
