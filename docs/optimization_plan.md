...
| 版本范围 | 支持状态 | 说明 |
|---------|---------|------|
| **Python 3.11 - 3.13** | ✅ 完全支持 | 推荐生产环境使用 |
| **Python 3.14** | ✅ 已适配 | 自动启用新特性(子解释器、t-string) |
| **Python 3.10 及以下** | ❌ 不支持 | 需要升级 Python 版本 |

**不支持 Python 3.10 及以下的原因**:
1. **类型注解依赖**: Python 3.11+ 的 `asyncio.TimeoutError` 是 `TimeoutError` 的别名,框架代码直接依赖此特性
2. **性能特性**: Python 3.11+ 的 asyncio 性能提升 2-4 倍,是框架高性能的基础
3. **依赖库要求**: 核心依赖(aiohttp>=3.13.3, pydantic>=2.11.7)要求 Python 3.11+
4. **兼容成本过高**: 使用版本守卫和回退机制会增加代码复杂度,降低可维护性
...
# 如果版本 < 3.11,请升级到 Python 3.11 或更高版本
# 推荐使用 pyenv 管理多版本:
# macOS/Linux:
...
> **重要**: 在安装 Crawlo 之前,请确保 Python 版本 >= 3.11,否则 `pip install crawlo` 会因 `python_requires >= 3.11` 而失败。
...
### 5.7 Fire-and-Forget 通知任务泄漏风险 — **优先级上调至 P0**

> **优先级修正**（2026-05-11 二次评估）：评估报告认为此问题直接影响系统可靠性（异常被静默丢弃、关闭时任务无法取消），应从 P2 提升至 **P0**。方案 A 实施成本仅 2 小时，收益/风险比极高。
...
### 5.9 Processor 队列空检查竞态

**问题**：`Processor.process_once()` 中 `queue.empty()` 和 `get_nowait()` 之间存在竞态窗口（虽然 asyncio 单线程下实际触发概率极低，但代码逻辑不严谨）：
...
**优化方案**：直接使用 `get_nowait()` + 捕获 `QueueEmpty` 异常，消除竞态窗口：
...
> **评估**：极低成本，1 分钟改动。优先级 P3（代码健壮性改进）。

---

### 5.10 Redis 过滤器错误处理不当

**问题**：`AioRedisFilter` 在网络错误时返回 `False`，可能导致重复请求：

```python
# aioredis_filter.py:278-281
except Exception as e:
    self.logger.error(f"Request check failed: {request.url} - {e}")
    return False  # 网络错误时返回 False → 爬虫会认为"未访问过"并重复抓取
```

**优化方案**：
1. 网络错误时返回 `True`（保守策略：宁可漏抓，不可重复）
2. 或添加降级策略：Redis 不可用时自动切换到内存去重（`MEMORY_FILTER_ON_REDIS_FAILURE=True`）
3. 添加重试逻辑（单次重试即可）

> **评估**：中等成本，影响数据准确性。优先级 P2（分布式场景下尤其重要）。
...
| **P0** | 5.7 Notify 任务泄漏 | 可靠性 | **低** | fire-and-forget 引用追踪，2hr 完成 |
| **P1** | 3.1 中间件日志开销 | 性能 | **极低** | isEnabledFor 守卫，实际收益 5-10% |
| **P1** | 3.2 Scheduler 队列等待 | 性能 | **中** | Condition 替代轮询 |
| **P1** | 4.1 过度防御性编程 | 代码质量 | **低** | 提取辅助方法 |
| **P1** | 5.5 Scheduler 返回值统一 | 可靠性 | **低** | 已基本统一，需全面排查 + 与 3.2 合并 |
...
| **P2** | 5.10 Redis 过滤器错误处理 | 可靠性 | **低** | 网络错误时应保守返回 True |
...
| **P3** | 5.9 Processor 队列空检查竞态 | 代码质量 | **极低** | 1 分钟改动，消除竞态窗口 |
...
| **优先级上调** | **5.7: P2→P0** | **影响系统可靠性，2hr 可完成** | **二次评估确认** |
| 优先级下调 | 5.5: P0→P1 | 返回值已基本统一，需全面排查 | 二次评估确认 |
...
| 收益修正 | 3.1: 30-50% → 5-10% | Python logger 内部已有 isEnabledFor | 二次评估确认 |
| 新增 | — | 5.9, 5.10 | 评估报告额外发现 |
...
| 5.7 Notify fire-and-forget 引用追踪（`_background_tasks` + `add_done_callback`） | 2 hr | 低 |
| 3.1 中间件日志添加 `isEnabledFor` 守卫 | 15 min | 极低 |
| 5.9 Processor 队列空检查改为 `get_nowait()` + `QueueEmpty` 异常捕获 | 5 min | 极低 |

**产出**：6 个零风险/低风险的代码改进，可立即提交。
...
| 5.5 全面排查 `enqueue_request()` 返回值 + 与 3.2 合并 | 3 hr | 无 | 低 |
| 5.10 Redis 过滤器网络错误处理（返回 True 或降级到内存去重）| 2 hr | 无 | 低 |
...
| 5.8 Redis 去重 Lua 脚本原子操作 | 1 天 | 仅分布式场景需要，建议提前到 P1 |
...
2. **可靠性**：生命周期管理存在边缘 Bug（4.3），资源清理有幂等缺陷（5.1），notify 任务缺乏引用追踪（**5.7 已提升至 P0**），Redis 过滤器错误处理不当（5.10）
...
| 5.7 Notify 任务泄漏影响系统可靠性 | 原 P2 优先级偏低 | ⬆️ **提升至 P0** |
| 5.5 返回值已基本统一 | 原 P0 问题严重程度偏高 | ⬇️ **降至 P1** |
| 3.1 日志收益 Python 已内部优化 | 原 30-50% 收益过高 | ⬇️ **修正为 5-10%** |
...
| Processor 队列空检查有竞态窗口 | 原方案未提及 | 🆕 **新增 5.9** |
| Redis 过滤器错误处理不当 | 原方案未提及 | 🆕 **新增 5.10** |
...
4. ~~5.7~~ Notify fire-and-forget 引用追踪 + 异常回调（2 hr）
5. ~~3.1~~ 日志守卫一行改动（15 min）
6. ~~5.9~~ Processor 队列空检查改为异常捕获（5 min）
