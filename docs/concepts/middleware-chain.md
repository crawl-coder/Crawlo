...
**优先级规则**（实际执行序，由 arch 守护测试锁定）：
- 数字越大，越靠外层
- `process_request` 按优先级**降序**执行（数字大者先处理请求）
- `process_response` 按优先级**升序**执行（数字小者先处理响应）

**插入位置对照表**（自定义中间件选值参考）：

| 想插在哪一层 | 推荐数值 | 说明 |
|---|---|---|
| 请求端最早介入（统一改写 URL / 签名认证） | ≥ 650 | 最外层，先于一切内置请求逻辑 |
| 反爬簇之前、站外过滤之后 | 401~450 | 可假定 URL 已通过 Offsite 检查，且先于浏览器渲染 |
| 默认自定义档（`MiddlewarePriority.CUSTOM`） | 500 | 位于 Retry 簇与反爬簇之间（框架默认档位） |
| 紧贴下载器前最后修改（末刻 header/代理） | 100~250 | 之后仅剩下载器 |

> ⚠ **响应端警告**：响应阶段数值小者先执行。若自定义中间件取值小于 600，
> 它会先于 `RetryMiddleware` 看到错误状态码——请勿在此丢弃可重试状态码
> （502/503 等），否则重试机制失效。同理，CloudflareBypass(355) 需要在
> 响应端看到挑战页原始响应，请勿在 355 之前按状态码丢响应。
