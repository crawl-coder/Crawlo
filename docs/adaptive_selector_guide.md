...
自适应追踪基于 **7 个维度**计算加权相似度：

1. **文本内容** (权重: 2.0) - 使用 SequenceMatcher 模糊匹配，最稳定的特征
2. **关键属性** (权重: 2.0) - `id`, `class`, `href`, `src` 等核心属性
3. **属性集合** (权重: 1.5) - 所有非排除属性的键值对匹配
4. **父节点特征** (权重: 1.0) - 父节点的标签、属性及文本
5. **DOM 路径** (权重: 1.0) - 元素在文档树中的绝对位置
6. **标签名** (权重: 1.0) - 元素的 HTML 标签
7. **兄弟节点** (权重: 0.5) - 前后兄弟节点的标签序列（变化较多，权重较低）

**加权平均算法**：即使某些维度（如 Class 或层级）发生剧烈变化，只要核心特征（如文本或 ID）保持一致，系统依然能以高置信度找回目标。

### ⚡ 高性能架构

- **LRU 内存缓存**：`FingerprintStorage` 内置了最近最少使用（LRU）缓存层。频繁读取的指纹将直接从内存返回，毫秒级响应，极大减轻了后端数据库（SQLite/Redis）的压力。
- **并发安全控制**：通过 `RLock` 和 `asyncio.Lock` 保护关键区域，确保在多线程或高并发异步环境下的数据一致性。
- **预过滤机制**：匹配前优先进行标签名过滤，显著缩小在大规模 DOM 树中的搜索范围。
