...
Crawlo 支持根据业务场景灵活选择部署模式，从单机开发测试到分布式大规模生产环境。
...
| `CRAWLO_MODE` | `"auto"` | 部署模式。`standalone`（内存模式）、`distributed`（分布式系统）、`auto`（多节点协作）。 |
| `CONCURRENCY` | `8` | 单个爬虫的并发请求数。控制同时抓取的网页数量。 |
| `MAX_RUNNING_SPIDERS` | `1` / `10` | 同时运行的最大爬虫数量。内存模式默认 1，分布式系统模式默认 10。 |
