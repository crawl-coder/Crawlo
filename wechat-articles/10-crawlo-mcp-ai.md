...
### 4. evaluate — 执行 JavaScript

让 AI 在浏览器中执行 JavaScript，提取动态渲染的数据：

```
你：帮我执行 document.querySelector('.price').innerText

AI：调用 evaluate(url="https://example.com", script="document.querySelector('.price').innerText")
  → 返回 JavaScript 执行结果
```

适用于：SPA 页面数据提取、表单字段获取、页面状态检测。
...
AI：调用 screenshot(url="https://example.com/login")
  → 返回页面截图（base64 PNG），AI 分析截图内容
```

适用于：验证码识别、页面布局分析、UI 兼容性检查。
...
> 这个 SPA 页面的数据在 JS 里，帮我提取出来

**Claude 的操作：**

```
→ 调用 evaluate(url="...", script="JSON.parse(document.getElementById('__DATA__').textContent)")
→ 返回 JSON 数据
→ AI 分析数据结构，生成解析代码
```
...
```
→ 调用 screenshot(url="https://example.com/login")
→ 返回 base64 PNG 截图
→ AI 视觉分析：数字验证码，4 位（AI 识别具体数字）
```
...
3. **AI**：JS 引擎执行复杂数据提取 + 截图验证
4. **AI**：生成完整的 Spider 代码
5. **你**：审查、运行、入库

内置 Prompt 模板（`scrape_prompt` / `data_collection_prompt`）让 AI 开箱即用，Cookie 自动持久化（`~/.crawlo/mcp_cookies.json`）保持登录态跨会话。
